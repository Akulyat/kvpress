...
class KVComposePress(BasePress):
    ...
    def get_important_per_layer(self):
        ...
        self.composite_scores_per_layer[0] = self.composite_scores_per_layer.max(dim=0).values  # Ensures first layer is the largest.
    ...
    def prepare_important_masks(self):
        ...
        self.important_mask_per_kv_head = [
            [torch.zeros(size=(self.context_len, ), device=self.device, dtype=torch.bool) for _ in range(self.num_kv_heads)]
            for _ in range(self.num_layers)
        ]

        for layer in range(self.num_layers):    
            for kv_head in range(self.num_kv_heads):
                count_of_important = self.important_per_layer[layer] if self.structured else self.important_per_head[layer, kv_head]
